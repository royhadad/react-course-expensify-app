...
function getObjectCloneWithoutProp(obj, prop) {
...
test('should not set amount on invalid input change', () => {
    const value = '12.122';
    const wrapper = shallow(<ExpenseForm />);
    const initialStateAmount = wrapper.state('amount');
    wrapper.find('input').at(1).simulate('change', {
        target: { value }
    });
    expect(wrapper.state('amount')).toBe(initialStateAmount);
});
...
test('should call onSubmit prop for valid form submission', () => {
    ...
    expect(onSubmitSpy).toHaveBeenLastCalledWith(getObjectCloneWithoutProp(expense, 'id'));
});
